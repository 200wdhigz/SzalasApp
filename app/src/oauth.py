...
def google_callback():
    ...
    try:
        ...
        given_name = user_info.get('given_name')  # Imię
        family_name = user_info.get('family_name')  # Nazwisko
        ...
        if session.get('oauth_linking'):
            ...
            # Powiąż konto Google z aktualnym użytkownikiem i zaktualizuj imię/nazwisko
...
def microsoft_callback():
    ...
    try:
        ...
        given_name = user_info.get('givenName')  # Imię
        surname = user_info.get('surname')  # Nazwisko
        ...
        if session.get('oauth_linking'):
            ...
            # Powiąż konto Microsoft z aktualnym użytkownikiem i zaktualizuj imię/nazwisko
            link_microsoft_account(current_user_id, microsoft_id)
            # Aktualizuj imię i nazwisko jeśli są dostępne
            if given_name or surname:
                update_user_name(current_user_id, first_name=given_name, last_name=surname)
...
def account():
    ...
    return render_template('account.html', user=user, logs=logs)
...
@oauth_bp.route('/account/update-name', methods=['POST'])
@login_required
def update_name():
    ...
    try:
        # Aktualizuj imię i nazwisko w Firestore
        update_user_name(user_id, first_name=first_name or None, last_name=last_name or None)
        flash('Imię i nazwisko zostały zaktualizowane.', 'success')
    except Exception as e:
